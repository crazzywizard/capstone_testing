  ...
  const mint = async (e) => {
    ...
    try {
      ...
      if (await isSkillMinted(skillId)) {
        const data = await getMintedData(skillId);
        console.log(data);
        alert(
          JSON.stringify({ tokenId: data.id.toString(), owner: data.owner, skillId: data.skillId })
        );
      } else {
    ...
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        <h1 className={styles.title}>
          Welcome to <a href="https://alexa4musicians.com/">Alexa4Musicians!</a>
        </h1>
        {onBoard ? (
